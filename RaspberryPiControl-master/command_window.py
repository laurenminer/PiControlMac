...
gst_location = "/Library/Frameworks/GStreamer.framework/Commands/gst-launch-1.0"
...
######################################
	...
	def view_stream(self):
		...
		receive_string = gst_location + " tcpclientsrc host=%s port=%s ! application/x-gdp, payload=96 ! gdpdepay ! rtph264depay ! decodebin ! autovideosink" %(self.pi.IP_ADDRESS,str(self.port))
		import subprocess
